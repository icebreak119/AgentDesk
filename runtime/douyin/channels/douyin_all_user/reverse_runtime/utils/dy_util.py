...
class _SilentPopen(_ORIGINAL_POPEN):
    """Windows 下隐藏子进程窗口；保持 ``Popen`` 为 class，避免破坏 asyncio/httpx。"""
...
def _bootstrap_execjs_node_env() -> str:
    env_path = _Path(__file__).resolve().parent / "execjs_node_env.py"
    spec = importlib.util.spec_from_file_location("dy_execjs_node_env", str(env_path))
    if not spec or not spec.loader:
        return ""
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.ensure_node_for_execjs()


_bootstrap_execjs_node_env()
import execjs


def _reverse_runtime_root() -> str:
    return str(path.dirname(path.dirname(path.abspath(__file__))))


def _static_dir() -> str:
    return path.join(_reverse_runtime_root(), "static")


def _execjs_workdir(static_dir: str) -> str:
    node_modules = path.join(static_dir, "node_modules")
    if path.isdir(node_modules):
        return node_modules
    if path.isdir(static_dir):
        return static_dir
    raise FileNotFoundError(f"execjs static dir missing: {static_dir}")


def _compile_execjs(script_name: str):
    static_dir = _static_dir()
    script_path = path.join(static_dir, script_name)
    with open(script_path, "r", encoding="utf-8") as fp:
        source = fp.read()
    return execjs.compile(source, cwd=_execjs_workdir(static_dir))


login_js = _compile_execjs("login.js")
dy_js = _compile_execjs("dy_ab.js")
sign_js = _compile_execjs("dy_live_sign.js")
