...
class Params:
    ...
    def with_auth_tokens(self, auth=None, url="", fake=False):
        if auth is None:
            return self
        self.with_web_id(auth, url, fake=fake)
        ms_token = getattr(auth, "msToken", "")
        if ms_token:
            self.add_param("msToken", ms_token)
        cookie = getattr(auth, "cookie", {}) or {}
        verify_fp = cookie.get("s_v_web_id") if isinstance(cookie, dict) else ""
        if verify_fp:
            self.add_param("verifyFp", verify_fp)
            self.add_param("fp", verify_fp)
        return self

    def with_web_defaults(self, auth=None, url="", params=None, fake_webid=False):
        self.with_platform()
        if params:
            self.update_params(params)
        self.with_auth_tokens(auth, url, fake=fake_webid)
        return self
